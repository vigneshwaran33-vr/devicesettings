 ...
 * multiple-app mode.
 * Eg: API to get the current video resolution : dsError_t dsGetResolution ( intptr_t handle, dsVideoPortResolution_t *resolution )
 *
 * @par IARM Support
...
class Manager {
	...
	virtual ~Manager();
public:
	static void Initialize();  //!< This function is used for out-of-process ds-client initialization and handles both the initialization and loading processes.
