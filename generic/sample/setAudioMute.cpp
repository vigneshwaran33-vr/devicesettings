...
int main(int argc, char *argv[]) 
{
    ...
    if (argc != 4) {
        printf("%s : <Port Type - HDMI, Component> <Port Number-0, 1, 2...> <Mute-1 or Unmute-0>\r\n", argv[0]);
        return 0;
    }

    char *portType = argv[1];
    char *portId = argv[2];
    bool toMute = (bool) atoi((const char *)argv[3]);

    try {
		printf("Sample Application: set Audio Mute\r\n");
		device::VideoOutputPort vPort = device::Host::getInstance().getVideoOutputPort(std::string(portType).append(portId));
		vPort.getAudioOutputPort().setMuted(toMute);
